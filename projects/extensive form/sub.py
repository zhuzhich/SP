...
def exp_cost_rule(model):
	...
	othIdr = sum(sum(model.pCCR[i]*(1-0.5*sum(model.u[i,t,r]+model.v[i,t,r] for t in model.sT))-\
					...
					for i in model.sI)
	setup  = sum(model.pd*model.zs[t] for t in model.sT)
	return idr1+othIdr+setup
model.oSub = Objective(rule=exp_cost_rule,sense=minimize)
