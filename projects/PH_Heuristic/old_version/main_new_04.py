	...
	for t1 in t1_v:
		...
		#main loop
			...
			while heuInfo.currentWin[0][0] <= T-1:
				...
				for pole_i in range(I):			#the smallest opportunity.
					...
					for i in range(j + 1, I):
						if heuInfoTenta.currentWin[i][0] -\
							...
							if t - shiftT < T:
								penaltyCost += sysInfo.comInfoAll[heuInfoTenta.currentWin[i][1]].cPR;
...
def main(wScaleBound, setUpCost, crBound, ranSeed ):
	...
	nComponents = 4		#number of components
	nStages = 20			# t = 1, 2, ..., T
...
wScaleL = [1, 5]
wScaleNormal = [1,8]
wScaleVector = [wScaleNormal]#[wScaleH, wScaleL]
dVector = [5]#[100, 5]
cCrH = [17, 27]
cCrL = [6, 16]
