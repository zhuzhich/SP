...
scen_list = [2]
counter = 0
directory = "C:\\Users\\zzhu3\\Documents\\codes\\SP\\projects\\rolling_horizon\\roll_benders" 
t_init = 2
for I in comp_list:
	for T in time_list:
		for w in scen_list:
			x = [-1]*I
			for t_roll in range(t_init, T+1):
				#excmd = "python genFile.py " + str(I) + " " + str(T) + " " + str(w)
				#os.system(excmd)
				counter += 1
				print (str(counter) + "_" + str(I) + "_" + str(T) + "_" + str(w))
				genFile.create_allFiles(I,T,w,x,t_roll,directory)
				# import the master a
				...
				print (cut_num)
				for i in mstr_inst.sI:
					print("x["+str(i)+"]="+str(round(mstr_inst.x[i](), 2)))
					x[i-1] = round(mstr_inst.x[i](), 2)						#update x for rolling horizon
				print ("objective value=%f" %mstr_inst.oMaster())
				for p in mstr_inst.Scen:
					print("objective valule ["+str(p)+"]"+str(round(mstr_inst.sita[p].value, 4)))
