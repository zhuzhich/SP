...
model.randSeed  = Param()

model.ps 		= Param(model.sI)
model.pKesi 	= Param(model.sI)
model.pCCR 		= Param(model.sI)
model.pCPR 		= Param(model.sI)
model.w_shape 	= Param(model.sI)
model.w_scale 	= Param(model.sI)

'''
model.ss		= Set()								#age
model.sKesi		= Set()								#failure state
model.sCCR		= Set()
model.sCPR		= Set()
model.s_w_shape	= Set()
model.s_w_scale	= Set()
...
def ps_init(model, i):
	return res[i-1];
model.ps = Param(model.sI, initialize=ps_init);
...
def pLT_init(model, i, r, w):
	random.seed(i+r+w+model.randSeed)
	if r == 1:
		if model.pKesi[i]==1:
			return 0
	#return i #for test only..
			...
			LT1 = int((ran_num_log**s_inv)*model.w_scale[i]) - model.ps[i]					
			LT = max(1,LT1)
			return LT
	else:	
		ran_num = random.uniform(0,1)
